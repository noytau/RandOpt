...
- **Free GPUs cannot be read from `runai describe node`** — it shows hardware capacity only, and other projects' allocations are invisible to `runai list jobs -p raja`. The cluster is split into node pools; the truth about free capacity is in the **Unschedulable events** of a pending pod (`runai describe job <name>`). Prefer submitting **without `--node-type`** and letting the scheduler place; a type pin can strand jobs in Pending even when same-type hardware looks idle (bitten 2026-07-14: A5000 pin pended while unpinned submits ran immediately).
