...
- **Unified conclusion (E1–E4):** the thicket phenomenon is absent from DINOv2's SSL landscape. Good models exist (E2 needle) but have negligible measure; that measure favours no task axis (E3) and does not grow with task-adaptation (E4). A defensible departure from the paper's implicit "task-adapted models sit in thickets." Open lever: **structured (non-isotropic) perturbation** — gradient/Fisher-subspace or low-rank ΔW sampling — the only untested way to reach the low-measure good set by random search.
